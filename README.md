...
YM2203 / YM2413 の4チップはRP2040標準クロックだと実機検証でリアルタイム処理が追いつかない
...
- **Pico** — ハードウェア基盤である Raspberry Pi Pico / Pico 2(RP2040 / RP2350)から。
- **Chiptune** — レトロなサウンドチップ(SN76489・YM2612・YM2151 など)による音楽・音源ジャンルを
  指す、既に定着した用語です。単に「Chip」だけだと一般的な(マイクロ)チップ全般とも読めて
  紛らわしく、しかも本プロジェクトは物理的なMCUチップ自体も何枚も使う構成なので、曖昧さの無い
  「Chiptune」をあえて選んでいます。
- **Orchestra** — 1チップにつき1枚の専用基板(マスター+チップごとのスレーブ)がそれぞれの
  パートを受け持ち、SPIバスで束ねられて1曲を合奏する、という本プロジェクトの分散マルチMCU
  構成そのものを表しています。
...
だけならビルド手順は読み飛ばして構いません。ボードごとに[firmware/pico1/](firmware/pico1/)
(RP2040)と[firmware/pico2/](firmware/pico2/)(RP2350)にフォルダを分けてあります。
...
(YM2612/YM2151/YM2203/YM2413)は実機検証でRP2040標準クロックでは処理が追いつかない
...
| `src/slave_ym2413/slave_ym2413.uf2` | **Pico 2 (RP2350)**※ (YM2413) |
...
cmake --build ~/build-vgmplay-pico2 --target slave_ym2612 slave_ym2151 slave_ym2203 slave_ym2413 -j$(nproc)
```
...
そのまま載せ替えられます。YM2612/YM2151/YM2203はオーバークロック(320MHz等)も必要ですが、
YM2413はPico 2の標準クロック(150MHz)のままで足ります。詳細・オーバークロック設定・RP2040の
ままで粘る場合の選択肢は[firmware/pico2/README.md](firmware/pico2/README.md)参照。
...
戻ります。`vgmplay.ini`の`[player] recursive = yes`でカード内の全フォルダを再帰的に走査し、
フォルダごとに全ファイルを再生する構成にもできます(既定はルート直下のみ)。`root_dir = <パス>`で
スキャンの開始位置をルート以外の特定フォルダに変更することもでき、`recursive = yes`と組み合わせる
と「そのフォルダの中だけを再帰的に再生する」構成にできます。`vgmplay.ini`で
